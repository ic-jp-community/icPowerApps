...
{
    public partial class UserControlExternalLinkManager : UserControl
    {
        public const string title = "外部リンク マネージャ";
        ...
        /// コンストラクタ
        /// </summary>
        public UserControlExternalLinkManager(IZBaseApp ironcadApp)
        {
            InitializeComponent();
        ...
        /// <param name="e"></param>
        private async void UserControlExternalLinkManager_VisibleChanged(object sender, EventArgs e)
        {
        ...
        private void buttonExecute_Click(object sender, EventArgs e)
        {
            ...
            /* 選択しているElementを取得する */
            List<IZElement> elemList = new List<IZElement>();
            getSelectedAssemblyPartElement(ref elemList);

            /* 選択Elementがあるかチェックする */
            if(elemList.Count <= 0)
            {
                MessageBox.Show("アセンブリ/パーツが選択されていません。");
                return;
            }

            /* 実行前に変更するElementがあるかを確認する */
            List<IZElement> changeElementList = new List<IZElement>();
            if (radioButtonExternalLink.Checked == true)
            {
                /* 外部リンクにする場合 */
                changeElementList = getExternalLinkedElement(true, elemList);
            }
            if (radioButtonExternalUnlink.Checked == true)
            {
                /* 外部リンクを解除する場合 */
                changeElementList = getExternalLinkedElement(false, elemList);
            }
            if (changeElementList.Count <= 0)
            {
                MessageBox.Show("変更するアセンブリ/パーツはありません。既に外部リンクまたは解除が設定済みです。");
                return;
            }
            ...
            sb.AppendLine("次のアセンブリ/パーツを変更します。");
            foreach(IZElement elem in changeElementList)
            {
                sb.AppendLine(string.Format("     {0}", elem.Name));
            }
            DialogResult dret = MessageBox.Show("外部リンクファイルの設定/解除を実行しますか？\n"+ sb.ToString(), "確認", MessageBoxButtons.YesNo);
            if(dret != DialogResult.Yes)
            {
                return;
            }
            ...
            {
                /* 自動でファイル名を付けるか */
                if (checkBoxOptionSaveFileNameUserName.Checked == true)
                {
                    autoFileName = true;
                }

                /* 自動でファイル名を付ける場合は保存先を事前に問う */
                if (autoFileName == true)
                {
                    string dirPath = string.Empty;
                    bool ret = icapiCommon.OpenDirectoryDialog("保存先を選択してください", ref dirPath);
                    if (ret != true)
                    {
                        return;
                    }
                ...
                foreach (IZElement elem in changeElementList)
                {
                    ...
                    if (autoFileName == true)
                    {
                        saveFilePath = Path.Combine(saveDirectoryPath, name + ".ics");
                        saveFilePath = getAutoFileName(saveFilePath);
                    }
                    else
                    {
                        SaveFileDialog sfd = new SaveFileDialog();
                        sfd.Title = string.Format("{0} の保存", elem.Name);
                        sfd.Filter = "IRONCAD Sceneファイル(*.ics)|*.ics|すべてのファイル(*.*)|*.*";
                        sfd.RestoreDirectory = true;
                        sfd.FileName = name;
                        dret = sfd.ShowDialog();
                        if (dret != DialogResult.OK)
                        {
                            MessageBox.Show("現在のファイルはスキップします。");
                            continue;
                        }
                        saveFilePath = sfd.FileName;
                    }
                    if (elem.Type == eZElementType.Z_ELEMENT_ASSEMBLY)
                    {
                        IZAssembly asm = elem as IZAssembly;
                        asm.SaveAs(saveFilePath, true);
                    }
                    if (elem.Type == eZElementType.Z_ELEMENT_PART)
                    {
                        IZPart parts = elem as IZPart;
                        parts.SaveAs(saveFilePath, true);
                    }
                }
            }
            ...
            if (radioButtonExternalUnlink.Checked == true)
            {
                ...
                bool warn = false;
                scene.UnlinkSceneElements(changeElementList.ToArray(), warn);
            }

            MessageBox.Show("外部リンクファイルの設定/解除を実行しました。");
        }
        ...
        /// <returns></returns>
        private string getAutoFileName(string filePath)
        {
            int number = 1;
            string newFilePath = string.Empty;
            string checkFilePath = filePath;
            string ext = Path.GetExtension(checkFilePath);
            string fileName = Path.GetFileNameWithoutExtension(checkFilePath);
            string path = Path.GetDirectoryName(checkFilePath);
            do
            {
                if (File.Exists(checkFilePath) != true)
                {
                    newFilePath = checkFilePath;
                    break;
                }
                checkFilePath = Path.Combine(path, string.Format("{0}_{1}{2}", fileName, number, ext));
                number++;
            } while (true);
        ...
        private void getSelectedAssemblyPartElement(ref List<IZElement> elemList)
        {
            elemList.Clear();
            IZDoc doc = this._ironcadApp.ActiveDoc;
            IZSceneDoc scene = doc as IZSceneDoc;
            ...
            if (list != null && list.Count > 0)
            {
                for (int i = 0; i < list.Count; i++)
                {
                    if ((list[i].Type == eZElementType.Z_ELEMENT_ASSEMBLY) ||
                        (list[i].Type == eZElementType.Z_ELEMENT_PART))
                    {
                        elemList.Add(list[i]);
                    }
                }
        ...
        /// <summary>
        /// 外部リンクになっているElementのみを取得する
        /// </summary>
        /// <param name="elemList"></param>
        private List<IZElement> getExternalLinkedElement(bool withOutExternal, List<IZElement> elemList)
        {
            List<IZElement> returnElementList = new List<IZElement>();

            foreach(IZElement elem in elemList)
            {
                bool link = false;
                if (elem.Type == eZElementType.Z_ELEMENT_ASSEMBLY)
                {
                    IZAssembly asm = elem as IZAssembly;
                    asm.GetExternallyLinkedInfo(out link);
                }
                else if(elem.Type == eZElementType.Z_ELEMENT_PART)
                {
                    IZPart part = elem as IZPart;
                    part.GetExternallyLinkedInfo(out link);
                }
                if (withOutExternal == true)
                {
                    if (link != true)
                    {
                        returnElementList.Add(elem);
                    }
                }
                else
                {
